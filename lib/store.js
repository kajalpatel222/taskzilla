...
function addTask(text) {
  if (!text || !text.trim()) {
    return { ok: false, message: 'Error: task text is required. Usage: todo add <text>' };
  }
  ...
  tasks.push(task);
  saveTasks(tasks);
  return { ok: true, message: `Added task ${task.id}: ${task.text}` };
}
...
function parseId(raw) {
  ...
  if (!Number.isInteger(id) || String(id) !== String(raw).trim()) {
    return { ok: false, message: `Error: invalid task id "${raw}".` };
  }
  return { ok: true, id };
}
...
function findTask(tasks, id) {
  ...
  if (!task) {
    return { ok: false, message: `Error: no task found with id ${id}.` };
  }
  return { ok: true, task };
}
...
function completeTask(rawId) {
  ...
  const tasks = loadTasks();
  const found = findTask(tasks, parsed.id);
  if (!found.ok) return found;
  const task = found.task;

  if (task.done) {
    return { ok: true, message: `Task ${task.id} ("${task.text}") is already done.`, alreadyDone: true };
  }
  ...
  const message =
    template.replace('{text}', task.text) +
    `\nRank: ${getRank(stats.completedCount)} (${stats.completedCount} completed all-time)`;
...
function deleteTask(rawId) {
  ...
  if (!found.ok) return found;
  const task = found.task;
  const remaining = tasks.filter((t) => t.id !== parsed.id);
  saveTasks(remaining);
  return { ok: true, message: `Deleted task ${parsed.id}: ${task.text}` };
}

function renderStats(stats) {
  return `Rank: ${getRank(stats.completedCount)}\nTasks completed all-time: ${stats.completedCount}`;
}

function showStats() {
  console.log(renderStats(loadStats()));
}
